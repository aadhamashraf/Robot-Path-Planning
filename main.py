...
# , export_frontier , draw_button , draw_path , showDifferences_ExcutionTime
...
def draw_button(screen, text, x, y, width, height):
    ...
    if ' ' in text:
        ...
        screen.blit(line2, (x + (width - line2.get_width()) //
                    2, y + height // 2 + 5))
    else:
...
def draw_path(path):
    for node in path:
        pygame.draw.rect(
            screen, GREEN, (node[0] * CELL_SIZE, node[1] * CELL_SIZE, CELL_SIZE, CELL_SIZE))


def showDifferences_ExecutionTime(compareAlgos):
    sorted_compareAlgos = dict(
        sorted(compareAlgos.items(), key=lambda item: item[1]))

    plt.figure(figsize=(10, 10))
    plt.barh(list(sorted_compareAlgos.keys()),
             list(sorted_compareAlgos.values()))
...
def main():
    grid = [[Cell(x, y) for x in range(MAZE_WIDTH)]
    ...
    while running:
        ...
        draw_button(screen, 'BFS', button_x, button_y,
                    button_width, button_height)
        draw_button(screen, 'DFS', button_x, button_y +
                    button_gap, button_width, button_height)
        draw_button(screen, 'UCS', button_x, button_y + 2 *
                    button_gap, button_width, button_height)
        draw_button(screen, 'IDS', button_x, button_y + 3 *
                    button_gap, button_width, button_height)
        draw_button(screen, '-', button_x + button_width + 5,
                    button_y + 3 * button_gap, button_width/8, button_height)
        draw_button(screen, '+', button_x + button_width + 40,
                    button_y + 3 * button_gap, button_width/8, button_height)
        draw_button(screen, 'Greedy BFS', button_x, button_y +
                    4 * button_gap, button_width, button_height)
        draw_button(screen, 'A Star', button_x, button_y + 5 *
                    button_gap, button_width, button_height)
        draw_button(screen, 'Hill Climbing', button_x, button_y +
                    6 * button_gap, button_width, button_height)
        draw_button(screen, 'Simulated Annealing', button_x,
                    button_y + 7 * button_gap, button_width, button_height)
        draw_button(screen, 'Genetic Algos', button_x, button_y +
                    8 * button_gap, button_width, button_height)
        draw_button(screen, 'Compare Excution Time', button_x,
                    button_y + 9 * button_gap, button_width, button_height)
        ...
        if selected_algorithm == 'BFS':
            path, frontier, step_count, timeTaken = Uninformed_Search.bfs(
                start, goal, grid)
            draw_path(path)
            ...
            path, frontier, step_count, timeTaken = Heuristic_Search.a_star(
                start, goal, grid)
            draw_path(path)
            ...
            path, frontier, step_count, timeTaken = Uninformed_Search.ucs(
                start, goal, grid)
            draw_path(path)
            ...
            path, frontier, step_count, timeTaken = Heuristic_Search.greedy_bfs(
                start, goal, grid)
            draw_path(path)
            ...
            path, frontier, step_count, timeTaken = Local_Search.hill_climbing(
                start, goal, grid)
            draw_path(path)
            ...
            path, frontier, step_count, timeTaken = Local_Search.simulated_annealing(
                start, goal, grid)
            draw_path(path)
            ...
            path, frontier, step_count, timeTaken = Local_Search.genetic_algorithm(
                start, goal, grid)
            draw_path(path)
        ...
        screen.blit(lIds, (button_x+button_width+19, button_y +
                    3 * button_gap + button_height/3))
